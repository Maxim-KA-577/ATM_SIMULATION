...
def save_setting(root, pan, money, valid_period, number):
    ...
    if len(str(card_pan)) != 4 or len(str(card_number)) != 16 or len(str(card_valid_period)) != 4:
        ...
        if int(card_valid_period) // 100 > 12 or int(card_valid_period) // 100 < 1:
            ...
            root.destroy()
            create_window(card_pan, card_valid_period, card_number, card_money)

def create_window(card_pan, card_valid_period, card_number, card_money):
    ...
    btn_working = ttk.Button(text = "вставить кату", command = lambda: atm.set_card(card_draw))
